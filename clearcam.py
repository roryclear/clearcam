    ...
    if self.vod[cam_name]:
      ...
      command = [
          ...
          *(["-rtsp_transport", "tcp"] if is_rtsp else []),
          ...
          "-f", "hls",
          "-hls_time", "2",
          "-hls_list_size", "0",  # Keep ALL segments (unlimited)
          "-hls_playlist_type", "event",  # Event type: growing playlist, no ENDLIST
          "-hls_flags", "append_list+independent_segments",  # Remove delete_segments
          "-hls_segment_type", "fmp4",
          "-hls_fmp4_init_filename", "init.mp4",
      ...
      if self.count[cam_name] > 10:
        ...
        if (time.time() - self.last_live_check[cam_name]) >= 5:
          ...
          if link != self.src[cam_name]:
            ...
            self.hls_proc[cam_name], self.proc[cam_name] = self._open_ffmpeg(cam_name)
          if global_settings.userID and not self.vod[cam_name]: threading.Thread(target=lambda: (self.check_upload_link(cam_name) if True else None), daemon=True).start()
