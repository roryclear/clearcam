...
- (instancetype)init {
    ...
    if (self) {
        ...
        if (!savedPresets) {
            self.yolo_presets = [[NSMutableDictionary alloc] initWithDictionary:@{
            ...
            [[NSUserDefaults standardUserDefaults] setObject:self.yolo_presets forKey:@"yolo_presets"];
        } else {
            self.yolo_presets = [savedPresets mutableCopy];
        }
